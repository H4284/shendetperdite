      ...
      <Carousel opts={{ loop: true }} className="w-full">
        <CarouselContent className="ml-0">
          {slides.map((slide, index) => {
            const darkText = slide.textColor === "dark";
            return (
              <CarouselItem key={slide.id} className="pl-0">
                <div className="relative min-h-[320px] overflow-hidden md:min-h-[420px] lg:min-h-[520px]">
                  ...
                  />
                  <div className={darkText ? "absolute inset-0 bg-white/10" : "absolute inset-0 bg-black/40"} />
                  <div
                    className={`relative mx-auto flex min-h-[320px] max-w-7xl flex-col justify-end gap-3 px-4 py-12 md:min-h-[420px] lg:min-h-[520px] ${darkText ? "text-neutral-900" : "text-white"}`}
                  >
                    {index === 0 ? (
                    ...
                    <p
                      className={`max-w-xl text-sm md:text-base ${darkText ? "text-neutral-700" : "text-white/90"}`}
                    >
                      {slide.subtitle}
                    </p>
                    <Button
